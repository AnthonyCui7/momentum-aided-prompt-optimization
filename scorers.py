...
class GSM8KScorer:
    ...
    def __call__(self, predictor, prompts, data, agg='mean', max_threads=1):
        # Each prompt is evaluated over the data; returns accuracy per prompt
        def compute_scores(prompts_exs):
            out_scores = {}
            inputs = [(ex, predictor, prompt) for prompt, ex in prompts_exs]
            with concurrent.futures.ProcessPoolExecutor(max_workers=max_threads) as executor:
                futures = [executor.submit(self.predict_on_example, input) for input in inputs]
                for future in tqdm(concurrent.futures.as_completed(futures), total=len(futures), desc='GSM8K scorer'):
                    prompt, ex, pred = future.result()
                    gold = self.clean_answer(ex['label'])
                    pred_clean = self.clean_answer(pred)
                    is_correct = self.match(gold, pred_clean)
                    out_scores[f'{ex}-{prompt}'] = 1 if is_correct else 0
            return out_scores
